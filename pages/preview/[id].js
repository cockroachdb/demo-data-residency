            ...
            <Fragment>
              <div className='flex flex-col gap-4'>
                ...
                <div className='flex flex-col gap-2 mx-auto max-w-lg'>
                  <p className='flex gap-2 m-0 text-center'>
                    Art created by <strong>{query.data.results.username}</strong>
                    <time className='text-inherit'>{`| ${query.data.results.local_last_update}`}</time>
                  </p>
                </div>
              </div>
              <div className='grid lg:grid-cols-2 gap-8 lg:gap-16'>
                <div>
                  <div className='flex flex-col gap-2'>
                    <div className='flex gap-4'>
                      {query.data.results.headings.map((heading, index) => {
                    ...
                    <div className='relative'>
                      <CockroachLabsIcon
                        className={`absolute w-1/12 h-1/12 z-20 ${
                          query.data.results.global_values
                            ? query.data.results.global_values?.position?.classes
                            : 'top-2 left-2'
                        }`}
                      />
                      <SiloLogo
                        background={
                          query.data.results.global_values
                            ? query.data.results.global_values?.theme?.classes[1]
                            : 'fill-brand-white'
                        }
                        foreground={
                          query.data.results.global_values
                            ? query.data.results.global_values?.theme?.classes[0]
                            : 'fill-brand-black'
                        }
                      />
                      <div className='relative grid grid-cols-5 z-10'>
                        {query.data.results.local_values.shapes.map((shape, s) => {
                                ...
                                {paths.map((path, p) => {
                                  return (
                                    <path
                                      ...
                                      d={path}
                                      style={{
                                        fill: query.data.results.local_values.colors[s]
                                      }}
                                    />
                                  )
                                })}
                      ...
                      <Image
                        loader={imageLoader}
                        src={query.data.results.local_values.url}
                        alt={query.data.results.local_values.name}
                        width={500}
                        height={300}
                        priority
                        className='absolute m-0 top-0 z-0 w-full'
                      />
                      {query.data.results.global_values?.position ? (
                        <svg
                          viewBox='0 0 500 300'
                          xmlns='http://www.w3.org/2000/svg'
                        ...
                        >
                          <defs>
                            <pattern
                              id={`${query.data.results.global_values.pattern.id}`}
                              viewBox='0,0,200,200'
                              width='20%'
                              height='30%'
                            >
                              {query.data.results.global_values.pattern.paths.map((path, index) => {
                          ...
                          <rect
                            width={500}
                            height={300}
                            fill={`url(#${query.data.results.global_values.pattern.id})`}
                          />
                        </svg>
                      ) : null}
                    </div>
                    ...
                    <div className='flex gap-2 justify-between bg-depth-1 p-2 text-xs text-brand-evening-hush'>
                      <span className='text-inherit'>{`Photograph by: ${query.data.results.local_values.credit}`}</span>
                    </div>
                  </div>
                </div>
                ...
                <div>
                  <div className='flex flex-col gap-2'>
                    <div className='flex gap-4'>
                      {query.data.placeholder.headings.map((heading, index) => {
                        const { flag, regionId, region } = heading
                    ...
                    <div className='relative'>
                      <div className='absolute flex flex-col items-center justify-center  w-full h-full'>
                        ...
                        <strong className='text-xl'>{query.data.placeholder.region}</strong>
                        <small className='block'>{query.data.placeholder.message}</small>
                      </div>
                      <Image
                        loader={imageLoader}
                        src={query.data.placeholder.image.url}
                        alt={query.data.placeholder.image.name}
                        width={500}
                        height={300}
                        priority
